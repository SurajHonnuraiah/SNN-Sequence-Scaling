...
#  The Architectural Problems for SNNs in Modeling
...
def save_run_config():
    config = {
        "paper_title": "SNNs Are Not Transformers (Yet): The Architectural Problems for SNNs in Modeling Long-Range Dependencies",
...
Paper: *SNNs Are Not Transformers (Yet): The Architectural Problems for SNNs in Modeling Long-Range Dependencies*
...
validation accuracy threshold. In the raw results, `best_val_acc` is retained as a
backward-compatible column name for the validation accuracy at the best-validation-loss checkpoint. If the threshold is not reached, the condition is marked as
...
def train_one_model(
    ...
    # Backward-compatible name: best_val_acc denotes the validation accuracy
    # at the checkpoint selected by minimum validation loss. This is the
    # validation-only metric used for M*(S); held-out test accuracy is never
    ...
    train_info = {
        ...
        "val_acc_at_best_val_loss": best_val_acc,
        "mstar_selection_acc": best_val_acc,
...
def _current_checkpoint_signature():
    ...
    return {
        ...
        "seeds": SEEDS,
        "variants_to_run": VARIANTS_TO_RUN,
        "variant_configs": {k: asdict(v) for k, v in VARIANT_CONFIGS.items()},
        "hidden_by_variant": HIDDEN_BY_VARIANT,
        "accuracy_thresholds": ACCURACY_THRESHOLDS,
        "d_in": D_IN,
        ...
        "early_stopping_patience": EARLY_STOPPING_PATIENCE,
        "use_early_stopping": USE_EARLY_STOPPING,
        "min_val_improvement": MIN_VAL_IMPROVEMENT,
        "batch_size": BATCH_SIZE,
        "eval_batch_size": EVAL_BATCH_SIZE,
        "lr": LR,
        "weight_decay": WEIGHT_DECAY,
                ...
                for variant in VARIANTS_TO_RUN:
                    ...
                    for M in tqdm(TRAIN_SIZES, desc=f"{task}, S={S}, {VARIANT_DISPLAY_NAMES[variant]}", leave=False, disable=DISABLE_TQDM):
                        ...
                        row = {
                            ...
                            "val_acc_at_best_val_loss": train_info["val_acc_at_best_val_loss"],
                            "mstar_selection_acc": train_info["mstar_selection_acc"],
...
def compute_mstar(df: pd.DataFrame, threshold: float, selection_metric_col: str = "best_val_acc"):
    ...
    accuracy reaches the requested threshold. The default column, best_val_acc,
    is the validation accuracy at the checkpoint selected by minimum validation
    loss. Held-out test accuracy is retained only as a final performance metric
    and is not used to select M*(S).
    ...
    for (task, variant, seed, S), g in df.groupby(["task", "variant", "seed", "S"]):
        ...
        rows.append({
            ...
            "selection_metric_definition": "validation accuracy at best-validation-loss checkpoint",
